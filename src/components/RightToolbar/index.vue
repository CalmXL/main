<template>
  <div class="top-right-btn" :style="style">
    <el-row align="middle" class="menu-bar">
      <div v-if="search" class="item-ico" @click="toggleSearch()">
        {{ showSearch ? '收起' : '展开' }}
        <el-icon>
      ...
      <div v-if="refreshShow" class="item-ico" @click="refresh()">
        <el-icon>
          <Refresh />
        </el-icon> 刷新列表
      </div>
...
function dataChange(data: any) {
  for (const item in props.columns) {
    const { key } = props.columns[item as any]
