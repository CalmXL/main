  ...
  <PageContent>
    <template #top>
      ...
      <el-row :gutter="10" class="mb8">
        ...
        <el-col :span="1.5">
          <el-button v-hasPermi="['system:role:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete">
            删除
          </el-button>
        </el-col>
        <!-- <el-col :span="1.5">
    ...
    <!-- 表格数据 -->
    <el-table v-loading="loading" class="base-table" stripe height="100%" :data="roleList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="100" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-popover placement="left" :width="'auto'" trigger="hover">
            <template #reference>
              <PopoverBtn v-if="Number(scope.row.roleId) !== 1" />
            </template>
            <div>
              <el-button
    ...
    <!-- 分配用户弹窗 -->
    <UserSelectCascader v-model="showUserSelectDialog" :role-id="currentRoleId" @success="getList" />
  </PageContent>
</template>
...
import UserSelectCascader from './UserSelectCascader.vue'
...
function checkShowDeleteBtn(row: any) {
  const { roleId, roleKey } = row

  if (Number(roleId) === 1) return false
  if (['system_role', 'audit_role', 'secrecy_role'].includes(roleKey)) return false
  return true
}
